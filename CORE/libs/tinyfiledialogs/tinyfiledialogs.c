...
static void Hex2RGB(const char aHexRGB[8], unsigned char aoResultRGB[3]) {
...
static void RGB2Hex(const unsigned char aRGB[3], char aoResultHexRGB[8]) {
	if (aoResultHexRGB) {
		if (aRGB) {
...
void tfd_replaceSubStr(const char* aSource, const char* aOldSubStr,
                       const char* aNewSubStr, char*    aoDestination) {
...
int tfd_quoteDetected(const char* aString) {
	const char* p;
...
const char* tinyfd_getGlobalChar(const char* aCharVariableName)
/* to be called from C# (you don't need this in C or C++) */
{
...
int tinyfd_getGlobalInt(const char* aIntVariableName) /* to be called from C# (you don't need this in C or C++) */
{
...
int tinyfd_setGlobalInt(const char* aIntVariableName, int aValue)
/* to be called from C# (you don't need this in C or C++) */
{
	if (!aIntVariableName || !strlen(aIntVariableName))
		return -1;
...
static int powershellPresent(void) {
	...
	FILE*       lIn;
	const char* lString = "powershell.exe";
...
static int sizeUtf16From8(const char* aUtf8string) {
	return MultiByteToWideChar(CP_UTF8, MB_ERR_INVALID_CHARS,
	                           aUtf8string, -1, NULL, 0);
}


static int sizeUtf16FromMbcs(const char* aMbcsString) {
	return MultiByteToWideChar(CP_ACP, MB_ERR_INVALID_CHARS,
	                           aMbcsString, -1, NULL, 0);
}


static int sizeUtf8(const wchar_t* aUtf16string) {
	return WideCharToMultiByte(CP_UTF8, WC_ERR_INVALID_CHARS,
	                           aUtf16string, -1, NULL, 0, NULL, NULL);
}


static int sizeMbcs(const wchar_t* aMbcsString) {
...
static void wipefileW(const wchar_t* aFilename) {
	int   i;
	FILE* lIn;
...
static wchar_t* getPathWithoutFinalSlashW(
	...
	const wchar_t* aSource)       /* aoDestination and aSource can be the same */
{
	const wchar_t* lTmp;
	if (aSource) {
...
static void Hex2RGBW(const wchar_t aHexRGB[8], unsigned char aoResultRGB[3]) {
...
static void RGB2HexW(const unsigned char aRGB[3], wchar_t aoResultHexRGB[8]) {
	#if (defined(__cplusplus ) && __cplusplus >= 201103L) || (defined(__STDC_VERSION__) && __STDC_VERSION__ >= 199901L) || defined(__clang__)
		wchar_t const * const lPrintFormat = L"#%02hhx%02hhx%02hhx";
	#else
	const wchar_t* const lPrintFormat = L"#%02hx%02hx%02hx";
	#endif
...
static int quoteDetectedW(const wchar_t* aString) {
	const wchar_t* p;
...
static char* ensureFilesExist(char*       aDestination,
                              const char* aSourcePathsAndNames) {
...
static int __stdcall EnumThreadWndProc(HWND hwnd, LPARAM lParam) {
	...
	auto    lDialogTitle = (const wchar_t*)lParam;
...
static void hiddenConsoleW(const wchar_t* aString, const wchar_t* aDialogTitle, int aInFront) {
...
int tinyfd_messageBoxW(
	...
	const wchar_t* aMessage,       /* NULL or ""  may contain \n and \t */
	const wchar_t* aDialogType,    /* "ok" "okcancel" "yesno" "yesnocancel" */
	const wchar_t* aIconType,      /* "info" "warning" "error" "question" */
...
wchar_t* tinyfd_inputBoxW(
	...
	const wchar_t* aDefaultInput) /* L"" , if NULL it's a passwordBox */
{
...
wchar_t* tinyfd_saveFileDialogW(
	const wchar_t*         aTitle,                   /* NULL or "" */
	const wchar_t*         aDefaultPathAndOrFile,    /* NULL or "" */
	int                    aNumOfFilterPatterns,     /* 0 */
	const wchar_t* const * aFilterPatterns,          /* NULL or {"*.jpg","*.png"} */
...
BOOL CALLBACK BrowseCallbackProcW_enum(HWND hWndChild, LPARAM lParam) {
	...
	if (wcscmp(buf, L"SysTreeView32") == 0) {
		auto hNode = TreeView_GetSelection(hWndChild);
...
wchar_t* tinyfd_selectFolderDialogW(
	const wchar_t* aTitle,       /* NULL or "" */
	const wchar_t* aDefaultPath) /* NULL or "" */
{
	static wchar_t lBuff[MAX_PATH_OR_CMD];
	wchar_t*       lRetval;
...
static char* saveFileDialogWinGui(
	char*               aoBuff,
	const char*         aTitle,                   /* NULL or "" */
	const char*         aDefaultPathAndOrFile,    /* NULL or "" */
	int                 aNumOfFilterPatterns,     /* 0 */
	...
	const char*         aSingleFilterDescription) /* NULL or "image files" */
{
	...
	lTmpWChar = tinyfd_saveFileDialogW(
		...
		(const wchar_t**)lFilterPatterns, /*stupid cast for gcc*/
...
static void writeUtf8(const char* aUtf8String) {
	unsigned long lNum;
	void*         lConsoleHandle;
	wchar_t*      lTmpWChar;
	...
	const char*         aSingleFilterDescription) /* NULL or "image files" */
{
	...
	if ((!tinyfd_forceConsole || !(GetConsoleWindow() || dialogPresent()))
		...
		p = saveFileDialogWinGui(lBuff,
		                         ...
		                         (const char* const *)aFilterPatterns, aSingleFilterDescription);
	}
	else if (dialogPresent()) {
		if (aTitle && !strcmp(aTitle, "tinyfd_query")) {
			strcpy(tinyfd_response, "dialog");
		...
		/* obtain a pointer on the current content of tinyfd_inputBox */
		if (lPointerInputBox)
	...
	int                 aAllowMultipleSelects)    /* 0 or 1 */
{
	...
	if ((!tinyfd_forceConsole || !(GetConsoleWindow() || dialogPresent()))
		...
		p = openFileDialogWinGui(aTitle, aDefaultPathAndOrFile, aNumOfFilterPatterns,
		                         (const char* const *)aFilterPatterns, aSingleFilterDescription, aAllowMultipleSelects);
	}
	else if (dialogPresent()) {
		if (aTitle && !strcmp(aTitle, "tinyfd_query")) {
			strcpy(tinyfd_response, "dialog");
...
void tfd_colorChooser(
	...
	const unsigned char aDefaultRGB[3] = {128, 128, 128};
